...
function fitInvoicePreview() {
  ...
  const padding = 48; // padding around the paper
  const panelWidth = panel.clientWidth - padding;
  
  // Choose base width based on A4/A5 orientation
  let paperWidth = 794; // default A4 or A5-Landscape
  if (paper.classList.contains('size-a5-p')) {
    paperWidth = 560;
  }
  
  if (panelWidth < paperWidth) {
    const scale = panelWidth / paperWidth;
    paper.style.transform = `scale(${scale})`;
    paper.style.transformOrigin = 'top center';
    
    // Set a height on the scroll wrapper to prevent layout collapse
    const wrap = document.querySelector('.bill-scroll-wrap');
    if (wrap) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(fitInvoicePreview, 150);
...
function renderItemsTable() {
  ...
  S.items.forEach((item, idx) => {
    ...
    tr.innerHTML = `
      ...
      <td><input class="inp-qty"   type="number" value="${item.qty}"               min="0" step="any" inputmode="decimal" data-field="qty"  style="width:60px"></td>
      <td><input class="inp-uom"   type="text"   value="${safeEsc(item.uom)}"      placeholder="KG/BOX" data-field="uom" style="width:65px"></td>
      <td><input class="inp-rate"  type="number" value="${item.rate}"              min="0" step="any" inputmode="decimal" data-field="rate" style="width:80px"></td>
...
function wireAllEvents() {
  ...
  const hamburger = el('mobileHamburger');
  ...
  if (hamburger && sidebar && overlay) {
    ...
    // Close sidebar menu drawer on link clicks
  ...
  // ── MOBILE EDIT/PREVIEW TAB SWITCHING ──
  const tabForm = el('mobileTabForm');
  const tabPreview = el('mobileTabPreview');
  const appLayout = el('appLayout');
  
  if (tabForm && tabPreview && appLayout) {
    tabForm.addEventListener('click', () => {
      tabForm.classList.add('active');
      tabPreview.classList.remove('active');
      appLayout.classList.add('show-form');
    ...
    tabPreview.addEventListener('click', () => {
      ...
      appLayout.classList.remove('show-form');
      setTimeout(fitInvoicePreview, 50);
    });
  }

  // ── MOBILE BUTTON CLICKS BRIDGING ──
  el('btnPrintMobile')?.addEventListener('click', () => el('btnPrint')?.click());
  el('btnNewBillMobile')?.addEventListener('click', () => el('btnNewBill')?.click());
